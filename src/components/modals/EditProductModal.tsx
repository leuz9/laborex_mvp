...
interface EditProductModalProps {
  product: Product;
  onClose: () => void;
  onSubmit: (productId: string, productData: Partial<Product>) => Promise<void>;
  loading?: boolean;
}
...
const EditProductModal: React.FC<EditProductModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: product.startDate.toDate().toISOString().split('T')[0],
    endDate: product.endDate ? product.endDate.toDate().toISOString().split('T')[0] : '',
    metadata: {
      repository: product.metadata?.repository || '',
      technologies: product.metadata?.technologies || [],
      teamSize: product.metadata?.teamSize || 1,
      complexity: product.metadata?.complexity || 'medium',
      priority: product.metadata?.priority || 'medium',
      deliverables: product.metadata?.deliverables || [],
      dependencies: product.metadata?.dependencies || [],
      environments: {
        dev: product.metadata?.environments?.dev || '',
        staging: product.metadata?.environments?.staging || '',
        prod: product.metadata?.environments?.prod || ''
      },
      tags: product.metadata?.tags || [],
      client: product.metadata?.client || '',
      budget: {
        allocated: product.metadata?.budget?.allocated || 0,
        spent: product.metadata?.budget?.spent || 0,
        currency: product.metadata?.budget?.currency || 'EUR'
      }
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSubmit(product.id, productData);
      onClose();
    } catch (error) {
